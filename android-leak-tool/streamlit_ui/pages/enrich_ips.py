...
# timebased chart
    ...
    map_df['date_str'] = map_df['timestamp'].dt.date.astype(str)

    if 'latitude' in map_df.columns and 'longitude' in map_df.columns:
        if 'timestamp' in map_df.columns and map_df['timestamp'].notna().any():
                fig = px.scatter_mapbox(
                map_df,
                lat="latitude",
                lon="longitude",
                color="risk_level" if "risk_level" in map_df.columns else None,
                hover_name="ip",
            ...
            size_max=15,
            mapbox_style="carto-positron",
            title="Suspicious IPs (No Time Data)",
            color_discrete_map={
                'High Risk': 'red',
                'Other': 'gray'
                }
